...
public class MensajeDAO {
    ...
    public static void borrarMensajeBD(int idMensaje) {
        ...
        try ( Connection conexion = db_connect.getConnection()) {
            PreparedStatement ps = null;
            try {
                //creacion del delete, borra el elemento que cumpla con el idMensaje
                String query = "DELETE FROM mensajes WHERE id_mensaje=?";
                ps = conexion.prepareStatement(query);
                
                ps.setInt(1, idMensaje);
                ps.executeUpdate();
                System.out.println("El mensaje con el id:"+idMensaje+" ha sido borrado");
            } catch (Exception e) {
                System.out.println("El mensaje no se puedo borrar:\ne");
            }
        } catch (Exception e) {
            System.out.println("No se pudieron recuperar los mensajes:\n" + e);
        }
    ...
    public static void actualizarMensajeBD(Mensaje mensaje) {
        ...
        try ( Connection conexion = db_connect.getConnection()) {
            PreparedStatement ps = null;
            try {
                //creacion del update, actualiza el mensaje del elemento que cumpla con el idMensaje
                ...
                ps.executeUpdate();
                System.out.println("El mensaje con el id:"+mensaje.getIdMensaje()+" ha sido actualizado");
            } catch (Exception e) {
                System.out.println("El mensaje no se pudo actualizar:\ne");
            }
        } catch (Exception e) {
